...
%}
x = 1:10;
y1 = [-6.5572 -6.5563 -6.5555 -6.5545 -6.5535 -6.5522 -6.5513 -6.5506 -6.5494 -6.5487];
y2 = [-6.556 -6.5551 -6.5546 -6.5539 -6.5531 -6.5516 -6.5508 -6.55 -6.5494 -6.5489];
plot(x,y1,x,y2)
legend('qu','hui')
...
%%%%%%%%%%%%%%%
%{
curve = importdata('/Users/treasersmac/ZJU-ALL/_6_grand3_2/Sensor/Sensor expirements/20180627-1/curve.txt');
x = curve(:,3);%1
y = curve(:,1);%2
z = curve(:,2)+curve(:,6)*0.001;

data=unique([x(:),y(:),z(:)],'rows'); %???????????????
f=@(p,data)(data(:,1)-p(1)).^2+(data(:,2)-p(2)).^2+(data(:,3)-p(3)).^2-p(4)^2; %p??????????????
p=nlinfit(data,zeros(size(data,1),1),f,[0 0 0 1]')
hold on
plot3(data(:,1),data(:,2),data(:,3),'o')
[X,Y,Z]=meshgrid(linspace(-14,14)); % meshgrid??????,linspace????-14?14??????
V=(X-p(1)).^2+(Y-p(2)).^2+(Z-p(3)).^2-p(4)^2;
isosurface(X,Y,Z,V,0); %?? isovalue ????????????? V ???????
alpha .5;camlight;axis equal;grid on;view(3);
title(sprintf('(x-%f)^2+(y-%f)^2+(z-%f)^2=%f^2',p(1),p(2),p(3),p(4)))

r = 150.163243067464;
x0 = 6.66160220166463;
y0 = 0.540340793418887;
z0 = -149.813567747871;
dz=[]
for i=1:length(z)
    dz(i)=z(i)-(sqrt((r^2-(x(i)-x0)^2-(y(i)-y0)^2))+z0);
end
mean(dz)
mean(z)
dz=dz';
